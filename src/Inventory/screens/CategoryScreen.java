...
public class CategoryScreen {
	...
	private static void addNewCategoryScreen() {
		...
		try {
			while((currCat = CategoryHandler.checkIfCategoryExists(choice, 1, null)) == null && choice !=0 &&choice !=-1)
			{
		...
		case 0:
			try {
				CategoryHandler.addNewCategory(name, 1, fatherCategory);
		...
		default:
			...
			choice = Util.readIntFromUser(-1, -1);
			try {
				while((currCat = CategoryHandler.checkIfCategoryExists(choice, 2, fatherCategory)) == null && choice !=0 &&choice !=-1)
				{
					System.out.println("Invalid choice please insert again");
					choice = Util.readIntFromUser(-1, -1);
				}
			...
			case 0:
				try {
					CategoryHandler.addNewCategory(name, 2, fatherCategory);
			...
			default:
				...
				case 0:
					try {
						CategoryHandler.addNewCategory(name, 3, fatherCategory);
					} catch (SQLException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
	...
	private static void updateDeleteExistingCategoryScreen() {
		int choice = 0;
		String name = null;
...
//							break;
						...
						case 0:
							...
							name = Util.readStringFromUser();
							try {
								CategoryHandler.updateCategoryName(parentCategory, name);
							} catch (SQLException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
	...
	private static Category updateDeleteHelper(Category parentCategory, int level) {
		int choice;
		String name;
		Category currCat = null;
		
		try {
			CategoryHandler.PrintAllSubCategories(parentCategory.get_id(), level);
		} catch (SQLException e2) {
			// TODO Auto-generated catch block
			e2.printStackTrace();
		}
		...
		try {
			while ((currCat = CategoryHandler.checkIfCategoryExists(choice, level, parentCategory)) == null && choice != 0 && choice != -1) {
				System.out.println("Invalid choice, please choose again");
				choice = Util.readIntFromUser(-1, -1);
			}
		} catch (SQLException e) {
