...
public class ProductStockScreen {
	...
	private static void addNewStockProductScreen() {
		try {
		...
		try {
			ProductStockHandler.printAllProductInStock(prod_id);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private static void addNewStockProduct(int prod_id) {
		String date;
		int amount;
		...
		try {
			while(ProductStockHandler.checkIfProductStockExists(prod_id, date, 1, false))
			{
		...
		try {
			ProductStockHandler.addProductStock(productStock);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private static void updateExistingStockProduct(int prod_id) {
		String date;
		System.out.println("To proceed press 1\nTo return to the previous screen press 2");
		if(Util.readIntFromUser(1, 2) == 2)
		{
		...
		try {
			ProductStockHandler.UpdateProductStock(productStock);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private static void moveProductToStore() {
		...
		while(!isFinished)
		{
			...
			while(!isExist)
			{
				...
				{
					try {
						isExist = ProductHandler.checkIfProductExists(id);
			...
			if (!endOneChoice) {
				try {
					existProductInStock = ProductStockHandler.printAllProductInStock(id);
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			while(!endOneChoice)
			{
				...
				if (choice == 2) {
					...
					try {
						endOneChoice = ProductStockHandler.checkIfProductStockExists(id, date, 1, true) &&
								ProductStockHandler.checkIfThereEnoughInStock(id, date, amount) && (date.compareTo(currDate) > 0);
					} catch (SQLException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
		...
		if (stockTicket.get_items().size() > 0) {
			try {
				stockTicket.set_id(StockTicketHandler.createNewStockTicket(currDate));
				
				// For each ProductStock in this StockTicket, we update the amount in stock and delete it if the amount is 0
				StockTicketHandler.addStockTicketItems(stockTicket);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
